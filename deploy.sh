...
then
    ...
    ACCESS_TOKEN=$(sed /rollbar_access_token/\!d < app/config/parameters.yml | cut -d: -f2 | tr -d " \r\n")
    LOCAL_USERNAME=`whoami`
    REVISION=`git log -n 1 --pretty=format:"%H"`
    curl https://api.rollbar.com/api/1/deploy/ -F access_token=$ACCESS_TOKEN -F environment=$SYMFONY_ENV -F revision=$REVISION -F local_username=$LOCAL_USERNAME
    echo ""
